...
> **更新された位置づけ：**
> OTUは、単なる深海送気装置ではなく、**双方向螺旋型の海洋鉛直流調律ユニット**として捉えられる。
> 正回転では、深部送気、酸素供給、ナノバブルによる循環補助を行う。
> 逆回転では、冷たい深層水を汲み上げ、海面・表層を直接冷却する。
> 本構想は現時点では概念設計であり、エネルギー収支、流量制御、水質、生態系影響について検証が必要である。

以下のモード概要では、送気、冷却、拡散、プランクトン補助、エネルギー回収、生態系安全制御を混同しないように整理している。

## OTUモード概要

| モード | 方向・役割 | 主機能 | 注記 |
|---|---|---|---|
| 正回転送気モード | 下方向・深部 | 送気、酸素供給、ナノバブル循環補助 | 本来OTUの基本モード。海の呼吸と弱い鉛直混合を補助する |
| 逆回転冷却モード | 上方向・表層 | 深層冷水汲み上げ、海面・表層直接冷却 | 高水温時・必要時のみ運用する |
| 拡散モード | 広角・螺旋 | 希釈、混合、局所集中の回避 | 深層冷水や栄養塩を安全に拡散する補助モード |
| プランクトン補助モード | 条件付き・副次機能 | 地域適応型・非有害系プランクトン回復補助 | 単一種の一律放流ではなく、監視と検証が必要 |
| エネルギー回収 / OTEC補助モード | 補助系 | 一部エネルギー回収、補助電源 | 完全自己駆動ではない |
| エコセーフモード | 停止・低出力 | 監視、安全停止、生態系保護 | pH、DO、クロロフィル、赤潮兆候、センサー異常時に停止または低出力化 |
